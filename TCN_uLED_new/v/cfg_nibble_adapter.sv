...
`ifndef SYNTHESIS
            $display("[%0t] ADAPTER ISSUE_CMD wr=%0d addr=0x%0h wdata=0x%08h",
              $time, op_write_r, addr_shift_r, data_shift_r);
`endif
        ...
        WAIT_RDATA: begin
          if (ctrl_link_rvalid) begin
            ...
            state_r          <= SEND_RDATA;
`ifndef SYNTHESIS
            $display("[%0t] ADAPTER GOT_RDATA addr=0x%0h rdata=0x%08h",
              $time, addr_shift_r, ctrl_link_rdata);
`endif
          end
        end
